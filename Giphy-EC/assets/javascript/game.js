    ...
    $.ajax({
        ...
        method: 'GET'
          ...
          for(var i = 0; i < results.length; i++){
            ...
            var rating = $("<div>");
            rating.addClass("caption col-md-12 col-sm-12 col-xs-12");
            rating.text("Rating: "+results[i].rating);
            var source=$("<a>");;
            if(results[i].source_post_url !== ""){
              //source = $("<a>");
              source.addClass("caption col-md-12 col-sm-12 col-xs-12");
              source.css("word-wrap", "break-word");
              source.text("Source: "+results[i].source_post_url);
              source.attr("href", results[i].source_post_url);
              source.attr("target", "_blank");
              
            }
            //if parental on is not checked, display every result
            if($("#parentalOn")[0].checked==false){
              ...
              container.append(source);
                ...
                if(results[i].rating != "r" && results[i].rating !="pg-13"){
                  console.log(results[i].rating!="r");
                  //append the image and the caption to the container div
                  container.append(img);
                  container.append(rating);
                  container.append(source);
                  ...
                  img.attr("src","assets/images/blocked.png");
                  source.text("Source: Blocked");
                  source.attr("href", "");
                  container.append(img);
                  container.append(rating);
                  container.append(source);
